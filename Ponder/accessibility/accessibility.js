...
if (movieList) {
  ...
  movies.forEach((movie, index) => {
    const headingId = `movie-title-${index + 1}`;
    ...
    article.innerHTML = `
      <figure class="movie-image">
        <img src="${movie.imgSrc}" alt="${movie.imgAlt}" width="400" height="600">
      </figure>
      <div class="movie-content">
        <h3 id="${headingId}">${movie.title}</h3>
        <p><strong>Release Date:</strong> ${movie.date}</p>
        <p><strong>Recommended Age:</strong> ${movie.ages}</p>
        <p><strong>Genre:</strong> ${movie.genre}</p>
        <p><strong>Rating:</strong>
          <span class="rating" aria-label="${starCount} out of 5 stars">${movie.stars}</span>
        </p>
